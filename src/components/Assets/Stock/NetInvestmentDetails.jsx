...
function NetInvestmentDetails({ year, accountFilter, onClose, refreshData }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const params = { transaction_type: ["deposit", "withdrawal"] };
      ...
      try {
        ...
        console.error("Error fetching net investment details:", error);
      }
      setCurrentPage(1);
      setLoading(false);
    };
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "NetInvestment");

    XLSX.writeFile(workbook, `Net_Investment_History_${year}.xlsx`);
  };
      ...
      <div className="relative bg-gray-900 rounded-3xl p-4 sm:p-8 w-full max-w-[95vw] sm:max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-gray-800">
        <div className="shrink-0 flex justify-between items-center mb-6 pb-4 border-b border-gray-800">
          <h2 className="text-xl sm:text-2xl font-black text-indigo-400 tracking-tight">
            Net Investment Details <span className="text-gray-500 ml-2">/ {year}</span>
          </h2>
          <div className="flex gap-2">
            <button
              onClick={handleDownloadExcel}
              className="p-2 bg-gray-800 hover:bg-indigo-900/40 hover:text-indigo-400 text-gray-400 rounded-xl transition-all border border-gray-700"
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-800/80 text-gray-400 text-[10px] font-black uppercase tracking-widest">
                      <th
                        className="p-4 cursor-pointer hover:text-indigo-400 transition-colors"
                      ...
                      <th
                        className="p-4 text-center cursor-pointer hover:text-indigo-400 transition-colors"
                  ...
                  <tbody className="divide-y divide-gray-800 text-sm">
                    {paginatedTransactions.map((t) => (
                      <tr key={t.id} className="hover:bg-gray-800/40 transition-colors">
                        <td className="p-4 font-medium text-gray-400 capitalize">{t.transaction_type}</td>
                        ...
                        <td className={`p-4 text-center font-bold ${t.transaction_type === 'deposit' ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-3xl shadow-2xl p-6 sm:p-8 w-full max-w-[500px] transform transition-all">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest ml-1">Type</label>
                <select
...
export default NetInvestmentDetails;