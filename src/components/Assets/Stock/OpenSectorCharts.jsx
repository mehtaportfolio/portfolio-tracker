...
function PieBlock({ title, labels, values, valueFormatter, percentOfTotal = false, actualForTooltip, colorByLabel = {} }) {
  ...
  if (!labels.length) return <p className="text-gray-500">No Sector data available.</p>;

  return (
    <div className="border rounded-lg p-3 shadow w-full max-w-full sm:max-w-sm">
      <h3 className="text-sm font-semibold mb-2 text-yellow-300 text-center break-words">{title}</h3>
...
function SectorLegend({ labels, colorByLabel, percentages = [] }) {
  if (!labels.length) return null;

  return (
    <div className="border rounded-lg p-4 shadow bg-slate-900/60 w-full sm:w-64 lg:w-80">
      <h3 className="text-sm font-semibold text-yellow-300 mb-3">Sector Legend</h3>
        ...
        {labels.map((label, idx) => (
          <li key={label} className="flex items-center gap-3 justify-between">
            ...
            {percentages.length > 0 && (
              <span className="text-yellow-300 flex-shrink-0 ml-2">{(percentages[idx] || 0).toFixed(1)}%</span>
            )}
          </li>
        ))}
      </ul>
...
// - stockMaster: array with { stock_name, cmp, lcp, Sector }
export default function OpenSectorCharts({ transactions = [], stockMaster = [] }) {
  const charts = useMemo(() => {
    ...
    (transactions || []).forEach((t) => {
      ...
      const cat = (master.sector || "Uncategorized").toString();
    ...
    // 5) IRR% (simple non-annualized return% per Sector)
    ...
    // 6) XIRR% per Sector (annualized, bisection)
  ...
  const { cats, investedVals, marketVals, investedSharePct, profitSharePct, irrPct, xirrPct, profitValsRaw } = charts;
  ...
  return (
    <div className="p-4 w-full max-w-full sm:p-6">
      <div className="flex flex-col gap-4 sm:gap-6">
        ...
        <SectorLegend labels={cats} colorByLabel={sectorColorMap} />
      </div>
    </div>
  );
}