...
function SectorLegend({ labels, colorByLabel, percentages = [] }) {
  ...
  return (
    <div className="border rounded-lg p-4 shadow bg-slate-900/60 w-full sm:w-64 lg:w-80">
      <h3 className="text-sm font-semibold mb-3 text-center text-yellow-300">Sector Legend</h3>
...
export default function ClosesectorCharts({ transactions = [], stockMaster = [] }) {
  const charts = useMemo(() => {
    ...
    (transactions || []).forEach((t) => {
      ...
      const cat = (master.sector || master.Sector || "Uncategorized").toString();
