...
import { useETFDataOptimized } from "../../../hooks/useETFDataOptimized.js";
...
const ETF = () => {
  ...
  const { etfs: backendStocks, summary: backendSummary, fetchETFData } = useETFDataOptimized();
  ...
  useEffect(() => {
    ...
    const openOnlyStocks = (backendStocks || []).map((stock) => {
      const openTransactions = (stock.transactions || []).filter((txn) => !txn.sell_date);
...
const recalculateOpenStockTotals = (stock, filters) => {
...
useEffect(() => {
  ...
  // Fall back to backend summary when filters return nothing
  const displaySummary = filteredStocks.length > 0 ? localSummary : backendSummary ?? defaultSummary;
  ...
  const handleExportIndividualToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Individual_ETF_Holdings");
    XLSX.writeFile(workbook, `${selectedStock.stock_name}_ETF_Holdings_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  const handleExportToExcel = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "ETF_Holdings");
    XLSX.writeFile(workbook, `ETF_Holdings_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
...
<div className="mb-8 flex flex-col gap-4 bg-white/5 backdrop-blur-2xl p-6 rounded-[2.5rem] border border-white/10 shadow-2xl">
  ...
  <div className="flex gap-4 w-full items-center">
    ...
    >
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  readOnly
                  className="w-full bg-white/5 border border-white/10 text-blue-100/30 rounded-2xl px-5 py-4 focus:outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSellValues({ ...sellValues, sell_date: e.target.value })}
                    className="w-full bg-white/10 border border-white/10 text-white rounded-2xl px-5 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500/50 [color-scheme:dark]"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSellValues({ ...sellValues, sell_price: e.target.value })}
                    className="w-full bg-white/10 border border-white/10 text-white rounded-2xl px-5 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSellValues({ ...sellValues, sell_quantity: e.target.value })}
                  className="w-full bg-white/10 border border-white/10 text-white rounded-2xl px-5 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                />
              </div>
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button
                  ...
                  className="flex-1 bg-amber-600 hover:bg-amber-500 text-white font-bold py-4 rounded-2xl shadow-lg shadow-amber-600/20 transition-all"
                >
                  Confirm Sell
                </button>
              </div>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] transition-all duration-300">
          <div className="bg-slate-900/90 backdrop-blur-2xl p-10 rounded-[3rem] shadow-[0_0_50px_rgba(0,0,0,0.5)] w-full max-w-lg border border-white/10">
            <h2 className="text-2xl font-bold mb-8 text-white tracking-tight flex items-center gap-3">
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              {EDITABLE_FIELDS.map((field) => (
                <div key={field.key} className={field.key === 'account_name' ? 'col-span-2' : ''}>
                  <label className="block text-[10px] font-bold text-blue-100/40 uppercase tracking-widest mb-2 ml-1">{field.label}</label>
                  {field.type === "select" ? (
                    <select
                      value={editValues[field.key] || ""}
                      onChange={(e) => handleEditFieldChange(field.key, e.target.value)}
                      className="w-full bg-white/10 border border-white/10 text-white rounded-2xl px-5 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none cursor-pointer"
                    >
                      <option value="" className="bg-slate-900">Select {field.label}</option>
                      {(field.optionsSource === "accounts" ? accountOptions : field.options || []).map((opt) => (
