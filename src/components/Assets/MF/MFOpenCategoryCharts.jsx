...
// Props:
// - transactions: array of open MF txns with { fund_short_name, units, buy_nav, buy_date }
// - fundMaster: array with { fund_short_name, cmp, lcp, category }
export default function MFOpenCategoryCharts({
  transactions = [],
  fundMaster = [],
}) {
  ...
  const charts = useMemo(() => {
    ...
    (transactions || []).forEach((t) => {
      ...
      const cmp =
        master.cmp != null && master.cmp !== ""
          ? Number(master.cmp)
          : NaN;
      const lcp =
        master.lcp != null && master.lcp !== ""
          ? Number(master.lcp)
          : NaN;
      const nav =
        !isNaN(cmp) && cmp > 0
      ...
      if (market > 0)
        flowsByCat[cat].push({ amount: market, date: new Date() });
    });

    const cats = Array.from(
      new Set([...Object.keys(investedByCat), ...Object.keys(marketByCat)])
    );
    ...
    const maskedMarketVals = isTrialMode ? marketVals.map(() => 0) : marketVals;
