...
// Props:
// - transactions: array of open MF txns
// - fundMaster: array with { fund_short_name, cmp, lcp, amc_name }
export default function MFOpenAMCCharts({
  transactions = [],
  fundMaster = [],
}) {
  ...
  const charts = useMemo(() => {
    ...
    (transactions || []).forEach((t) => {
      ...
      marketByAmc[amc] = (marketByAmc[amc] || 0) + market;
      ...
      if (market > 0)
        flowsByAmc[amc].push({ amount: market, date: new Date() });
    });

    const amcs = Array.from(
      new Set([...Object.keys(investedByAmc), ...Object.keys(marketByAmc)])
    );

    const investedVals = amcs.map((a) => investedByAmc[a] || 0);
    const marketVals = amcs.map((a) => marketByAmc[a] || 0);
